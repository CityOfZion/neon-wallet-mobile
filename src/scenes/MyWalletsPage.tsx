...
const MenuItemComponent = (props: { navigation: StackNavigationProp<SettingsStackParamList>; wallet: Wallet }) => {
  return (
    <MenuItem
...
const MyWalletsPage = (props: Props) => {
  ...
  return (
    <ScreenLayout padding={20} darkerSolidColorBG>
      {isListNotEmpty() ? (
        wallets.map((it, i) => <MenuItemComponent key={i} navigation={props.navigation} wallet={it} />)
      ) : (
        <LinearLayout alignItems="center" width="100%" height="100%">
          <TextView color="white" fontSize={18} mt={9} fontFamily="medium" textAlign="center">
            {i18n.t('myWalletsPage.noWallet')}
          </TextView>
          <TextView color="white" fontSize={18} fontFamily="medium" textAlign="center">

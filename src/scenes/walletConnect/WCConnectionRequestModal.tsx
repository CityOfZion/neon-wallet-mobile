...
import { EStatus, TSessionProposal, useWalletConnectWallet } from '@cityofzion/wallet-connect-sdk-wallet-react'
...
const WCConnectionRequestModal = (props: Props) => {
  ...
  const proposalInformation = useMemo(() => {
    if (!proposal) return

    return WalletConnectHelper.getInformationFromProposal(proposal)
  }, [proposal])
  ...
  const handleFinishSelect = useCallback(
    async ({ account }: TOnFinishSelectionParams) => {
      try {
        ...
        const blockchain = WalletConnectHelper.convertBlockchain(account.blockchain)
        ...
        await approveProposal(proposal, { address: account.address, chain, blockchain })
  ...
  const navigateToSelection = useCallback(() => {
    ...
    props.navigation.navigate(wrapper.route.Modal.name, {
      ...
      params: {
        ...
        blockchainFilter: proposalInformation.blockchain,
        style: 'alter',
  ...
  const handleAccept = useCallback(async () => {
    if (!proposal || !proposalInformation) return

    const currentNetwork = selectedBlockchainNetworks[proposalInformation.blockchain].type
    const dAppNetwork = proposalInformation.network
    ...
    if (currentNetwork !== dAppNetwork) {
      showAlert({
        ...
        subtitle: i18n.t('modals.WCConnectionRequest.dialog_message', {
          ...
          dAppNetwork: i18n.t(`app.networks.${proposalInformation.network}`),
          currentNetwork: i18n.t(`app.networks.${selectedBlockchainNetworks[proposalInformation.blockchain].type}`),
        }),
        buttons: [
    ...
    navigateToSelection()
  }, [proposal, navigateToSelection, selectedBlockchainNetworks, proposalInformation])
  ...
  const handleSwitchNetwork = () => {
    ...
    const value = networks[proposalInformation.blockchain].find(network => network.type === proposalInformation.network)
    if (!value) {
      showMessage({
        message: i18n.t('walletconnect.alert.networkNotFound', { network: proposalInformation.network }),
        type: 'danger',
      })
      controller.close()
      return
    ...
    dispatch(
      settingsReducerActions.setSelectNetwork({
        blockchain: proposalInformation.blockchain,
        id: value.id,
      })
    )
              ...
              >
                <LinearLayout weight={1}>
                  ...
                  <LinearLayout orientation="horiz" mt="6px">
                    <BlockchainIcon blockchain={proposalInformation.blockchain} width={20} height={20} mr="2px" />

                    <TextView fontFamily="medium" color="#fff" fontSize="16px">
                      {i18n.t(`blockchainServices.${proposalInformation.blockchain}.id`)}
                    </TextView>
                  </LinearLayout>
                </LinearLayout>
                ...
                <LinearLayout weight={1}>
                  ...
                  {proposal.params.requiredNamespaces[proposalInformation.proposalBlockchain].methods.map(
                    (it, index) => (

...
type Props = {
  navigation: StackNavigationProp<
    TabStackParamList & RootStackParamList & ModalStackParamList
  >
...
const componentsByMethod: Record<
  SupportedMethods,
  React.FC<TransactionRequestMethodComponentProps>
> = {
...
const WCTransactionRequestModal = ({navigation, route}: Props) => {
  ...
  const account = useMemo(() => {
    const [{address}] = WalletConnectHelper.getAccountInformationFromSession(
      session
    )

    return accounts.find(
      (account) => account.address && account.address === address
    )
  }, [accounts])
  ...
  const Component =
    componentsByMethod[request.request.method as SupportedMethods]

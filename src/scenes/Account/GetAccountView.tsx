...
const GetAccountView = (props: GetAccountViewProps) => {
  ...
  const changeSenderAdress = () => {
    if (senderAdress === '' && address !== null) {
      setSenderAdress(address)
    }
  }
    ...
    if (address) {
      const account = dispatchAccount(
        RootStore.account.actions.getFromSelection(
          senderAdress !== '' ? senderAdress : undefined
        )
      )
      setAccount(account)
    }
    ...
    >
      ...
      <View
        ...
        onTouchMove={({nativeEvent}) => {
          if (nativeEvent.pageX > initAxisX) handleChangeScreen(true)

          if (nativeEvent.pageX < initAxisX) handleChangeScreen(false)
        }}
      >
        <TabSelector

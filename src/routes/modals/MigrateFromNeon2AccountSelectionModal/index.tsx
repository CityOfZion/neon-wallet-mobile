...
export const MigrateFromNeon2AccountSelectionModal = ({
  navigation,
  route,
}: TRootStackScreenProps<'MigrateFromNeon2AccountSelectionModal'>) => {
  const { content, onSuccess } = route.params

  const { t } = useTranslation('modals', { keyPrefix: 'migrateFromNeon2AccountSelection' })
  ...
  const handleNext = () => {
    navigation.navigate('MigrateFromNeon2PasswordModal', { accounts, content, onSuccess })
  }

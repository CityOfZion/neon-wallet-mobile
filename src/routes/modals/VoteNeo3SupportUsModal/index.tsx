...
export const VoteNeo3SupportUsModal = ({
...
}: TRootStackScreenProps<'VoteNeo3SupportUsModal'>) => {
  const { t } = useTranslation('modals', { keyPrefix: 'voteNeo3SupportUsModal' })
  ...
  const handleClose = () => {
    if (actionDataRef.current.dontShowAgain) dispatch(settingsReducerActions.dontShowVoteNeo3SupportUsModalAgain())
  ...
  const handleGoToVoteNeo3ConfirmationModalForCoz = async () => {
    handleClose()

    await UtilsHelper.sleep(500)

    navigation.navigate('VoteNeo3ConfirmationModal', { neo3Account, candidate: cozCandidate })
  }
    ...
    >
      ...
      <View className="my-2 flex flex-grow flex-col justify-end gap-y-6">
        <TwButton
          ...
          onPress={handleGoToVoteNeo3ConfirmationModalForCoz}
        />

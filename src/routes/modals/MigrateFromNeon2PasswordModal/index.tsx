...
import { useNeonMigrateFile } from '@/hooks/useNeonMigrateFile'
...
export const MigrateFromNeon2PasswordModal = ({
  navigation,
  route,
}: TRootStackScreenProps<'MigrateFromNeon2PasswordModal'>) => {
  const { accounts, content, onSuccess } = route.params

  const { t } = useTranslation('modals', { keyPrefix: 'migrateFromNeon2Password' })
  ...
  const handleImport = async (data: TActionsData) => {
    try {
      ...
      LoggerHelper.error(error, { where: 'MigrateFromNeon2PasswordModal', operation: 'handleImport' })

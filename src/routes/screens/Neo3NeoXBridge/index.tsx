...
export const Neo3NeoXBridgeScreen = ({
  navigation,
  route: {
    params: { account },
  },
  ...
  const { actionData, actionState, setData, reset, handleAct } = useActions<TActionsData>({
    ...
    accountToUse: { value: account || null, error: null, loading: false },
  ...
  const handleGoToNeo3NeoXBridgeAboutModal = () => {
    navigation.navigate('Neo3NeoXBridgeAboutModal')
  }
  ...
  const handleSetAccountToUse = async (account: TAccount<TBSBridgeName>) => {
    try {
      ...
      LoggerHelper.error(error, { where: 'Neo3NeoXBridgeScreen', operation: 'handleSetAccountToUse' })
  ...
  const handleSelectAccountToUse = () => {
    ...
    navigation.navigate('AccountSelectionModal', {
      ...
      blockchains: blockchain ? [blockchain] : [neo3Service.name, neoXService.name],
      onSelect: account => {
        handleSetAccountToUse(account as TAccount<TBSBridgeName>)
      },
    })
  }
  ...
  const handleSelectTokenToUse = () => {
    navigation.navigate('TokenSelectionModal', {
      ...
      onSelect: (token: TBridgeToken<TBSBridgeName>) => {
  ...
  const initializeOrRestartService = async () => {
    ...
    neo3NeoXBridgeOrchestrator.eventEmitter.on('accountToUse', accountToUse => {
      ...
      setData({ accountToUse: { ...accountToUse, value: value as TAccount<TBSBridgeName> | null } })
    })
  ...
  const handleSubmit = () => {
    ...
    navigation.navigate('Neo3NeoXBridgeConfirmationModal', {
      tokenToUse,
      tokenToReceive,
      accountToUse,
      addressToReceive,
      ...
      onSubmit: async modalNavigation => {
        try {
          ...
          modalNavigation.navigate('Neo3NeoXBridgeDetailsModal', {
            tokenToUse,
            tokenToReceive,
            accountToUse,
            addressToReceive,
          ...
          LoggerHelper.error(error, { where: 'Neo3NeoXBridgeScreen', operation: 'handleSubmit' })

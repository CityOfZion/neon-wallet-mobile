...
import { useVoteNeo3GetCandidatesToVote, useVoteNeo3GetVoteDetailsByAddress } from '@/hooks/useVoteNeo3'
...
const renderItem: ListRenderItem<TItem> = ({
  ...
  const tPrefix = 'screens:voteNeo3Screen.listItem'
...
export const VoteNeo3List = ({ neo3Account, search, voteErrorMessage, canVote, canScrollToCandidateRef }: TProps) => {
  const { t } = useTranslation('screens', { keyPrefix: 'voteNeo3Screen.list' })
  const { data, isLoading } = useVoteNeo3GetCandidatesToVote()
  ...
  const { filteredCandidates, votesTotal } = useMemo(() => {
    ...
    data.forEach(candidate => {
      ...
      if (
        ...
        candidate.pubKey === ConstantsHelper.voteNeo3CozPubKey ||
    ...
    () =>
      filteredCandidates.map<TItem>(candidate => {
        ...
        return {
          ...
          onGoToDetails: () => {
            navigation.navigate('VoteNeo3CandidateDetailsModal', {
              neo3Account,
              candidate,
              candidateVotePercentage,
            })
